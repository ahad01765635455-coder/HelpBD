  ...
  try {
    ...
    if (req.method === 'GET') {
      const data = await supabaseRequest(
        'donor_applications?select=id,name,phone,dob,birth_registration_no,blood_group,division,district,upazila,union_name,verification_status,status,created_at,updated_at&order=created_at.desc'
      );
    ...
    if (req.method === 'PATCH') {
      if (body.action === 'verify') {
        const data = await supabaseRequest(
          `donor_applications?id=eq.${safeId}`,
          { method: 'PATCH', body: JSON.stringify({ verification_status: 'verified' }), headers: { Prefer: 'return=representation' } }
        );
      ...
      if (body.action === 'unverify') {
        const data = await supabaseRequest(
          `donor_applications?id=eq.${safeId}`,
          { method: 'PATCH', body: JSON.stringify({ verification_status: 'not_verified' }), headers: { Prefer: 'return=representation' } }
        );
        return json(res, 200, { ok: true, donor: Array.isArray(data) ? data[0] : data });
      }
      ...
      if (body.action === 'approve') {
        ...
        if (current[0].verification_status !== 'verified') {
          return json(res, 400, { error: 'Verify the donor before approval' });
        }
        const data = await supabaseRequest(
          `donor_applications?id=eq.${safeId}`,
          { method: 'PATCH', body: JSON.stringify({ status: 'approved' }), headers: { Prefer: 'return=representation' } }
        );
        return json(res, 200, { ok: true, donor: Array.isArray(data) ? data[0] : data });
      }
